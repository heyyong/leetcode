...
export function longestPalindrome(s: string = ""): string {
  const n = s.length;
  const substrMatrix: number[][] = new Array(n)
    .fill(null)
  ...
  const getSlicePos = (
    ...
    column: number,
    length: number
  ): [number, number] => [row + 1 - length, row + 1];

  substrMatrix.map((rowValue, row) =>
    rowValue.map((columnValue, column) => {
      if (s[row] === s[reversePos(column) - 1]) {
        ...
        if (
          ...
          checkPalindrome(
            s,
            ...getSlicePos(row, column, substrMatrix[row][column])
          )
        ) {
          maxPos.row = row;
  ...
  return s.slice(
    ...getSlicePos(
      maxPos.row,
      maxPos.column,
      substrMatrix[maxPos.row][maxPos.column]
    )
  );
}
